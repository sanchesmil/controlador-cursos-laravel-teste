...
@section('conteudo')  <!-- Define o conteúdo da sessão 'conteudo' -->
    ...
    @include('mensagem', ['mensagem'=> $mensagem])   <!-- Chama a wiew 'mensagem' passando uma mensagem p/ ser exibida -->
    ...
    <ul class="list-group">
        ...
        <li class="list-group-item d-flex justify-content-between align-items-center">
            <span id="nome-serie-{{ $serie->id }}">{{ $serie->nome }}</span>   <!-- '{{'....'}}' = Sintaxe do blade  -->
            ...
            <div class="input-group w-50" hidden id="input-nome-serie-{{ $serie->id }}">
                ...
                <div class="input-group-append">
                    ...
                    @csrf   <!-- Converte-se em um input 'hidden' com o valor do TOKEN necessário ao Laravel p/ ele processar a requisição -->
                </div>
            </div>
            ...
            <span class="d-flex">
                 @auth    <!-- Mostra o botão de edição somente p/ usuários logados -->
                    ...
                    <!-- Botão de editar -->
                    <button class="btn btn-info btn-sm mr-1" title="Editar" onclick="toggleInput({{$serie->id}})">
                        <i class="fas fa-edit"></i>  <!-- Adiciona um icone de edicao de Font Awesome -->
                    </button>
                 @endauth

                <!-- Link para 'temporadas' -->
                <a href="/series/{{$serie->id}}/temporadas" class="btn btn-info btn-sm mr-1" title="Temporadas">
                ...
                @auth    <!-- Mostra o botão de exclusão somente p/ usuários logados -->
                    ...
                    <form action="/series/{{$serie->id}}" method="post" onsubmit="return confirm('Tem certeza que deseja remover {{addslashes($serie->nome)}}?')"> 
                        ...
                        @csrf <!-- Adicionar o @crsf para não ter problemas de segurança com o token -->

                        @method('DELETE')    <!-- Diretiva do blade que sobrescreve o método do form, tendo em vista que o HTML não suporta métodos diferentes de get ou post. Na prática, ele cria um campo hidden e a cada requisicao verifica sua existencia -->
    ...
    <script>
        ...
        function toggleInput(serieId){
            ...
            if(nomeSerieElem.hasAttribute('hidden')){       // 1° Mostra a div nome se estiver escondida (= hidden)
                ...
                nomeSerieElem.removeAttribute('hidden');    // Mostra a div nome removendo o atributo hidden
                inputSerieElem.hidden = true;               // Esconde o input de edição add o atrib hidden = true

            }else {                                         // 2º Esconde a div nome se estiver visivel
